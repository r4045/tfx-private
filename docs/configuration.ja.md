...
| `openBookmarkDialog` | `f8` | ブックマークのクイック ジャンプ ダイアログを開く。各項目に 2 文字のニーモニック（グループ文字＋項目文字）が付き、入力するとアクティブペインがそのフォルダーへ移動します。確定のキー（2 文字目または Enter）で Shift を押していると新しいタブで開きます。Esc で取り消し。 |
