...
| `syncCwd` | `ctrl+shift+j` | アクティブなファイル一覧を内蔵ターミナルの現在フォルダーに合わせる。シェルが起動していないときは何もしません。ターミナルペインにフォーカスがある間はキーがシェルに渡るため無視されます。先に `Ctrl+1` でファイル一覧に戻してから押してください。 |
| `quit` | `ctrl+q` | アプリを終了する（セッションを保存しターミナルを破棄）。ターミナルペインにフォーカスがある間はシェルの `Ctrl+Q` を優先するため無視されます。`Alt+F4` は常にウィンドウを閉じます。 |
| `changeMoveMode` | `f1` | 移動モード（Explorer / Vi）を切り替え。Vi モードでは `j`/`k`/`h`/`l` などの文字キーで移動し、標準の先頭一致検索（type-ahead）は無効になります。名前列ヘッダーとステータスバーのモード表示が赤くなります。モードは保存されず、起動時は常に Explorer です。 |
| `openExplorer` | `ctrl+shift+o` | 現在のフォルダーをエクスプローラーの新しいウィンドウで開く（項目は選択しません）。アーカイブ内を表示しているときは、そのアーカイブ ファイルが置かれているフォルダーを開きます。 |
| `moveClipboard` | `ctrl+shift+v` | クリップボードのパスへアクティブペインを移動。テキストのパスを優先し（複数行なら最初の空でない行）、なければファイルドロップ リストの先頭を使います。フォルダーならそのフォルダーを開き、ファイルなら親フォルダーを開いてそのファイルを選択します。 |
| `addBookmark` | `ctrl+d` | 現在のフォルダーをブックマークに追加。グループ / エイリアス / パスを入力するダイアログが開きます（グループ名が既存ならそこへ追加、新規なら作成）。アーカイブ内では無効です。 |
| `pathToClipboard` | `f9` | パスをクリップボードにテキストとしてコピー。選択があれば先頭 1 件のフルパス（`..` 行は無視）、何も選択していなければアクティブペインの現在のフォルダーのパスをコピーします。「先頭」は表示順ではなく選択順です。 |
| `nameToClipboard` | `shift+f9` | 選択項目の**名前だけ**（パスなし）をクリップボードにテキストとしてコピー。複数選択時は 1 行 1 件で全件コピーします（順番は表示順ではなく選択順）。`..` 行は無視します。何も選択していないときはアクティブペインの現在のフォルダー名をコピーします。 |
| `nameNoExtToClipboard` | `ctrl+shift+f9` | `nameToClipboard` と同じですが、**拡張子を除いた**名前をコピーします（`report.pdf` → `report`）。拡張子の除去は**ファイルのみ**です。フォルダー名の `.` は拡張子ではないため（例: `v1.2.3`）、フォルダーと現在のフォルダー名はそのままコピーします。 |
| `openBookmarkDialog` | `f8` | ブックマークのクイック ジャンプ ダイアログを開く。各項目に 2 文字のニーモニック（グループ文字＋項目文字）が付き、入力するとアクティブペインがそのフォルダーへ移動します。**確定のキー（2 文字目または Enter）で押していた修飾キーが動作を決めます**: なし＝アクティブペインで移動、`Shift`＝新しいタブで開く、`Ctrl`＝移動せずそのブックマークを編集（`Ctrl+Enter` または `Ctrl+2 文字目`）。`Ctrl` は `Shift` より優先されます。編集ではグループ / エイリアス / パスのダイアログが現在値入りで開き、グループを変更すると移動先グループの**末尾**へ移動し（存在しない名前なら新規作成）、空になった元グループは削除されます。移動先に同じパスが既にある場合は更新せず警告のみ表示します。Esc で取り消し。 |
| `commandPalette` | `ctrl+p` | コマンド パレットを開く。現在の状況で実行できる組み込みコマンドと、条件に合致する `[[commands]]` を一覧し、入力で絞り込んで Enter で実行します。 |
| `viCheatSheet` | `shift+f1` | Vi モードのキー一覧（チートシート：`j`/`k`/`h`/`l`、`g`/`G`、`a`/`n`、`c`/`x`/`p`）をダイアログで表示。`Enter` または `Esc` で閉じます。 |
| `featureList` | `ctrl+shift+f1` | 機能一覧ダイアログを開く（設定ボタンのメニューからも開けます）。組み込みアクションとユーザー定義 `[[commands]]` を、割り当てキーと説明付きで一覧します。Key 列のヘッダーをクリックすると昇順→降順→元の順に並べ替えられ、行をクリックすると選択状態になります。`Esc` で閉じます。 |
| `sortByModifiedFlat` | `shift+f3` | アクティブペインを更新日時の新しい順に並べ替え。フォルダーとファイルを分けずに混在させます（列ヘッダーのクリックでは作れない並び順）。列ヘッダーのクリックと同じ仕組みを使うため、フォルダー移動後も並び順は維持されます。 |
